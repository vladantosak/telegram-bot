...
def check_status(text: str, report_type: str | None = "status") -> dict:
    ...
    if is_forced:
        ...
        mode_instruction = (
            ...
            "Это не итог дня, а отчёт об одном рабочем эпизоде. Просто прислал позже, чем нужно.\n"
            "\n"
            "ИТОГ ДНЯ (daily_fact) — человек подводит итоги всего рабочего дня целиком.\n"
            "Смысл сообщения: «вот что мы сделали за сегодня в целом».\n"
            "\n"
            "Ключевой вопрос: человек описывает МОМЕНТ работы или ИТОГ ДНЯ?\n"
            "\n"
            "Примеры:\n"
            "«работал на кране, подавал материал» — это рассказ об одном эпизоде → status\n"
            "«копали траншею у склада» — один эпизод → status\n"
            "«сегодня сделали опалубку, арматуру и залили колонны» — итог нескольких работ за день → daily_fact\n"
            "«за день: земляные работы и монтаж плит» — итог дня → daily_fact\n"
            "\n"
