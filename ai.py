...
def clean_report(text: str) -> str:
    ...
    try:
        response = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "Ты чистишь речь рабочего от слов-паразитов и артефактов распознавания, сохраняя все факты и его собственные формулировки - не пересказываешь и не добавляешь ничего от себя. Верни только готовый текст без каких-либо комментариев и кавычек."},
